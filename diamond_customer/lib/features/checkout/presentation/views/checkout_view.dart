      ...
      child: BlocConsumer<CheckoutCubit, CheckoutState>(
        listener: (context, state) {
          if (state is CheckoutLoaded && state.message != null && state.message!.isNotEmpty) {
            ...
            AppToast.success(context, message: 'تم إرسال الطلب بنجاح!');
        ...
        builder: (context, state) {
          ...
          if (state is CheckoutLoading) {
            return Scaffold(
              ...
              appBar: const CustomAppBar(title: 'إتمام الطلب'),
              body: const Center(
          ...
          if (state is CheckoutLoaded || state is CheckoutPlacingOrder) {
            ...
            return Scaffold(
              ...
              body: ListView(
                ...
                children: [
                  // Issues Warning Banner if eligibility failed or issues exist
                  ...
                  Text('موقع التوصيل', style: AppTextStyles.headingSmall.copyWith(fontWeight: FontWeight.bold)),
                  SizedBox(height: 10.h),
                  CheckoutSectionCard(
                    icon: Icons.location_on_outlined,
                    label: address != null ? (address.label ?? 'عنوان التوصيل') : 'اختر عنوان التوصيل',
                    value: address != null ? address.fullAddressText : 'لم يتم اختيار عنوان تفصيلي بعد',
                    onChange: () async {
                      final selected = await context.push<Address>(AppRoutes.addressList);
                      if (selected != null) {
                        cubit.selectAddress(selected.id);
                      } else {
                        // Reload checkout in case default changed
                        cubit.loadCheckout();
                      }
                    },
                  ...
                  // Backend Provided Payment Options
                  Text('طريقة الدفع المناسبة', style: AppTextStyles.headingSmall.copyWith(fontWeight: FontWeight.bold)),
                  SizedBox(height: 10.h),
                  if (availablePaymentMethods.isEmpty)
                    Text('لا تتوفر طرق دفع حالياً', style: AppTextStyles.bodyMedium.copyWith(color: AppColors.textSecondary))
                  else
                    Column(
                  ...
                  // Delivery notes
                  Text('تعليمات التوصيل', style: AppTextStyles.headingSmall.copyWith(fontWeight: FontWeight.bold)),
                  SizedBox(height: 10.h),
                  AppTextField(
                    controller: _notesController,
                    hint: 'مثال: يرجى الاتصال عند الوصول أو ترك الطلب عند الباب',
              ...
              bottomNavigationBar: SafeArea(
                child: Container(
                  ...
                  child: AppButton(
                    label: isEligible
                        ? 'تأكيد وإرسال الطلب • ${total.toStringAsFixed(0)} ر.س'
                        : 'غير مؤهل لإتمام الطلب',
